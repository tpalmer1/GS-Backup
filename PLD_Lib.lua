...
pName = player.name
-- Saying hello
windower.add_to_chat(8,'----- Welcome back to your PLD.lua, '..pName..' -----')
...
\cs(255, 255, 64)${key_bind_cast}\cs(200, 200, 200)Cast:\cr \cs(125,125,255)${player_current_cast|Normal}
...
\cs(255,255,64)${key_bind_lunge} \cs(200, 200, 200)Lunge:\cr ${toggle_lunge|Normal} 
]]

hub_battle_std = [[ \cs(255, 115, 0)Battle: \cr
\cs(255,255,64)${key_bind_death} \cs(200, 200, 200)Death Resist:\cr ${death_resist|OFF} 
	]]

-- LITE Mode
hub_mode_lte = [[ \cs(255, 115, 0) == Modes: \cr              \cs(255, 255, 64)${key_bind_idle} \cs(200, 200, 200)Idle:\cr \cs(125,125,255)${player_current_idle|Refresh}              \cs(255, 255, 64)${key_bind_melee} \cs(200, 200, 200)Melee:\cr \cs(125,125,255)${player_current_melee|Refresh}              \cs(255, 255, 64)${key_bind_cast} \cs(200, 200, 200)Cast:\cr \cs(125,125,255)${player_current_cast|Normal}               \cs(255, 255, 64)${key_bind_mainweapon} \cs(200, 200, 200)Main / Sub Weapon:\cr \cs(125,125,255)${player_current_mainweapon|Crocea Mors} / ${player_current_subweapon|Ammurapi Shield}]]


hub_options_lte = [[ \cs(255, 115, 0)== Options: \cr              \cs(255, 255, 64)${key_bind_lock_weapon} \cs(200, 200, 200)Lock Weapon:\cr ${toggle_lock_weapon}            \cs(255, 255, 64)${key_bind_movespeed_lock}\cs(200, 200, 200)Carm Cuisse:\cr ${toggle_movespeed_lock} ]]

hub_job_lte = [[ \cs(255, 115, 0) == ${player_job}: \cr             \cs(255, 255, 64)${key_bind_rune} \cs(200, 200, 200)Rune:\cr ${rune_color|\\cs(0, 204, 204)}${toggle_rune_cycle|Ice}              \cs(255,255,64)${key_bind_lung} \cs(200, 200, 200)Lunge:\cr ${toggle_lunge|Normal} ]]

hub_battle_lte = [[ \cs(255, 115, 0) == Battle: \cr             \cs(255,255,64)${key_bind_death}\cs(200, 200, 200)Death Resist:\cr ${death_resist|OFF}  ]]
...
keybinds_off['key_bind_lunge'] = '        '
keybinds_off['key_bind_death'] = '        '
...
function validateTextInformation()
	...
	if deathresist.value then
		main_text_hub.death_resist = const_on
	else
		main_text_hub.death_resist = const_off
	end
	...
	main_text_hub.player_current_cast = castModes.current
	main_text_hub.toggle_lunge = lungeModes.current
...
function update_active_ja(name, gain)
    Buff['Cover'] = buffactive['Cover'] or false
...
function midcast(spell)
    ...
    if spell.name:match('Cure') or spell.name:match('Cura') then
        equip(sets.midcast.cure[castModes.current])
	...
	elseif spell.skill == 'Divine Magic' then
		if spell.name:match('Flash') then
			equip(sets.me.enmity[castModes.current])
		elseif spell.name:match('Enlight') then
			equip(sets.midcast.divine.potency)
		end
	-- Blue Magic
	elseif spell.type == 'BlueMagic' then
		...
		elseif blueMap == 'enm' then
			equip(sets.midcast.blue.enm[castModes.current])
		else
...
function idle()
    ...
    if player.status=='Engaged' then
        if subWeapon.current:match('Grip') then
