...
function precast(spell)
    ...
    if spell.action_type  == 'Magic' and spell_recasts[spell.recast_id] > 0 then
        ...
        if spell.skill == "Elemental Magic" then
            downgradenuke(spell)
            add_to_chat(8, '****** ['..spell.name..' CANCELLED - Spell on Cooldown, Downgrading spell...] ******')
        else
...
function midcast(spell)
    ...
    elseif spell.skill == 'Enhancing Magic' then
        ...
        elseif spellMap == "Gain" then
            equip(sets.midcast.enhancing.duration,{hands=RELIC.Hands})
        else
...
function self_command(command)
    ...
    if #commandArgs:split(' ') >= 2 then
        ...
        if commandArgs[1] == 'toggle' then
            ...
            elseif commandArgs[2] == 'rangeweapon' then
                if meleeing.value then
                    rangeWeapon:cycle()
                    idle()
                    if use_UI == true then
